...
#include "../../build/ImagePayload/script/builtin.h"
#endif
...
void ShowCustomImage(){
    ...
    //Get the image size
    FILINFO finfo;
    FRESULT img_info = f_stat("sd:/asturcon.bmp", &finfo);
    if (img_info){
    ...
    FIL img;
    FRESULT img_open = f_open(&img, "sd:/asturcon.bmp", FA_READ);
    if (img_open){
        gfx_printf("Error opening image file!");
        return;
    }

    u32 img_size = finfo.fsize;
    u8 *img_buf = malloc(img_size);
    if (!img_buf){
    ...
    u32 br;
    FRESULT img_read = f_read(&img, img_buf, img_size, &br);
    if (img_read){
    ...
    //Get the BMP header
    bmp_t *bmp = (bmp_t*)img_buf;
    uint32_t* img_data = (uint32_t*)(img_buf + bmp->data_off);

    //Draw the image pixel by pixel
    for (int y = 0; y < bmp->height; y++){
        for (int x = 0; x < bmp->width; x++){
            uint8_t g = (img_data[y * 1280 + x] >> 16) & 0xFF;
            uint32_t color = (g << 24) | (g << 16) | (g << 8) | g;
            gfx_set_pixel(719 - y, 1279 - x, color);
        }
    }
}
